  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex items-center justify-between">
        <div>
          <h1 className="text-lg font-semibold text-text">Policy Engine & Limits</h1>
        ...
        <div className="flex gap-2">
          ...
          <button className="flex items-center gap-1 rounded-md bg-purple px-3 py-1.5 text-[10px] font-semibold text-bg hover:bg-purple/90">
      ...
      <div className="rounded-lg border border-border bg-bg-raised">
        <div className="flex items-center gap-2 border-b border-border px-4 py-3">
          ...
          <span className="text-xs font-semibold text-text">Active Rules</span>
        ...
        <div className="divide-y divide-border">
          {rules.map((rule) => (
            <div key={rule.id} className="px-4 py-3">
                ...
                <div className="flex items-center gap-3">
                  <button
                    ...
                    className={`flex h-5 w-5 items-center justify-center rounded border ${
                      rule.enabled
                        ? "border-purple/40 bg-purple-dim"
                  ...
                  <div>
                    <div className="text-xs font-medium text-text">{rule.name}</div>
                    <div className="text-[10px] text-text-muted">{rule.description}</div>
                  </div>
                </div>
                <div className="flex items-center gap-3">
                  <span className="font-mono text-xs text-purple">{rule.value}</span>
                  <button
              ...
              {editing === rule.id && (
                <div className="mt-3 ml-8 rounded-md border border-border bg-bg-surface p-3">
                  <input
                    type="text"
                    defaultValue={rule.value}
                    className="w-full rounded border border-border bg-bg-raised px-2 py-1.5 font-mono text-xs text-text outline-none focus:border-purple/40"
                  />
                  <div className="mt-2 flex gap-2">
                    <button className="rounded bg-purple px-3 py-1 text-[10px] font-semibold text-bg">Apply</button>
                    <button onClick={() => setEditing(null)} className="rounded px-3 py-1 text-[10px] text-text-muted">Cancel</button>
                  </div>
                </div>
              )}
            </div>
      ...
      <div className="grid gap-4 lg:grid-cols-2">
        ...
        <div className="rounded-lg border border-border bg-bg-raised">
          <div className="flex items-center justify-between border-b border-border px-4 py-3">
            <span className="text-xs font-semibold text-text">Vendor Whitelist</span>
            <button
              onClick={() => setShowAddVendor(true)}
              className="flex items-center gap-1 rounded bg-purple-dim px-2 py-1 text-[10px] text-purple hover:bg-purple/15"
            >
          ...
          <div className="divide-y divide-border">
            {vendorList.map((v) => (
              <div key={v.name} className="flex items-center justify-between px-4 py-2">
                <div className="text-xs text-text">{v.name}</div>
                <div className="flex items-center gap-3">
                  <span className="font-mono text-[10px] text-text-muted">{v.spend}</span>
        ...
        <div className="rounded-lg border border-border bg-bg-raised">
          <div className="flex items-center gap-2 border-b border-border px-4 py-3">
            ...
            <span className="text-xs font-semibold text-text">Policy Audit Log</span>
          </div>
          <div className="divide-y divide-border">
            {auditLog.map((log, i) => (
              <div key={i} className="px-4 py-2">
                <div className="flex items-center justify-between">
                  <div className="flex items-center gap-2">
                    <span className="font-mono text-[9px] text-text-muted">{log.time}</span>
                    <span className="text-[10px] text-text">{log.rule}</span>
                  </div>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60">
          <div className="w-full max-w-sm rounded-lg border border-border bg-bg-raised p-5">
            <div className="mb-4 flex items-center justify-between">
              <span className="text-sm font-semibold text-text">Add Vendor</span>
              <button onClick={() => setShowAddVendor(false)} className="text-text-muted hover:text-text">
                <X className="h-4 w-4" />
              </button>
            </div>
            <div className="space-y-3">
              ...
              />
              <div className="flex gap-2">
                <button onClick={() => setShowAddVendor(false)} className="flex-1 rounded-md border border-border bg-bg-surface py-2 text-xs text-text-muted">
                  Cancel
                </button>
                <button className="flex-1 rounded-md bg-purple py-2 text-xs font-semibold text-bg hover:bg-purple/90">
                  Add Vendor
                </button>
              </div>
            </div>
