"use client";

import { Shield, ExternalLink } from "lucide-react";

const team = [
  { name: "Saber", url: "https://github.com/winsznx" },
  { name: "Enoch", url: "https://github.com/ENOCH208" },
  { name: "Daniel", url: "https://github.com/danielAsaboro" },
  { name: "Huda", url: "https://github.com/PugarHuda" },
  { name: "Parth", url: "https://www.github.com/mittal-parth" },
];

export function Footer() {
  return (
    <footer className="border-t border-border bg-bg">
      <div className="mx-auto max-w-6xl px-6 py-12">
        <div className="grid gap-10 md:grid-cols-3">
          <div>
            <div className="flex items-center gap-2">
              <Shield className="h-4 w-4 text-text-muted" />
              <span className="text-sm font-semibold text-text">
                Omni<span className="text-purple">CFO</span>
              </span>
            </div>
            <p className="mt-3 max-w-xs text-xs leading-relaxed text-text-muted">
              Autonomous corporate treasury agent for the Syndicate hackathon.
              Track 2: Autonomous Office of the CFO.
            </p>
          </div>

          <div>
            <h4 className="text-xs font-semibold text-text">Resources</h4>
            <div className="mt-3 space-y-2">
              <a
                href="https://github.com/Saber1Y/OmniCFO"
                target="_blank"
                rel="noopener noreferrer"
                className="flex items-center gap-1.5 text-xs text-text-muted hover:text-text transition-colors"
              >
                <svg className="h-3.5 w-3.5" viewBox="0 0 24 24" fill="currentColor"><path d="M12 0c-6.626 0-12 5.373-12 12 0 5.302 3.438 9.8 8.207 11.387.599.111.793-.261.793-.577v-2.234c-3.338.726-4.033-1.416-4.033-1.416-.546-1.387-1.333-1.756-1.333-1.756-1.089-.745.083-.729.083-.729 1.205.084 1.839 1.237 1.839 1.237 1.07 1.834 2.807 1.304 3.492.997.107-.775.418-1.305.762-1.604-2.665-.305-5.467-1.334-5.467-5.931 0-1.311.469-2.381 1.236-3.221-.124-.303-.535-1.524.117-3.176 0 0 1.008-.322 3.301 1.23.957-.266 1.983-.399 3.003-.404 1.02.005 2.047.138 3.006.404 2.291-1.552 3.297-1.23 3.297-1.23.653 1.653.242 2.874.118 3.176.77.84 1.235 1.911 1.235 3.221 0 4.609-2.807 5.624-5.479 5.921.43.372.823 1.102.823 2.222v3.293c0 .319.192.694.801.576 4.765-1.589 8.199-6.086 8.199-11.386 0-6.627-5.373-12-12-12z"/></svg>
                GitHub Repository
              </a>
              <a
                href="#architecture"
                className="flex items-center gap-1.5 text-xs text-text-muted hover:text-text transition-colors"
              >
                <ExternalLink className="h-3.5 w-3.5" />
                Architecture Docs
              </a>
            </div>
          </div>

          <div>
            <h4 className="text-xs font-semibold text-text">Team</h4>
            <div className="mt-3 flex flex-wrap gap-2">
              {team.map((member) => (
                <a
                  key={member.name}
                  href={member.url}
                  target="_blank"
                  rel="noopener noreferrer"
                  className="flex items-center gap-1 rounded-md border border-border px-2 py-1 text-[10px] text-text-muted transition-all hover:border-purple/20 hover:text-text"
                >
                  {member.name}
                  <ExternalLink className="h-2.5 w-2.5" />
                </a>
              ))}
            </div>
          </div>
        </div>

        <div className="mt-10 flex flex-wrap items-center justify-between gap-3 border-t border-border pt-6">
          <div className="font-mono text-[10px] text-text-muted">
            Syndicate Hackathon 2026 - Track 2 Submission
          </div>
          <div className="font-mono text-[10px] text-text-muted">
            Open Source - MIT License
          </div>
        </div>
      </div>
    </footer>
  );
}
